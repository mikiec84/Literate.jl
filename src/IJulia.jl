...
function limitstringmime(mime::MIME, x)
    ...
    if istextmime(mime)
        if israwtext(mime, x)
            ...
            invokelatest(show, IOContext(buf, :limit=>true, :color=>true), mime, x)
        end
    else
        ...
        if isa(x, Vector{UInt8})
            ...
            invokelatest(show, IOContext(b64, :limit=>true, :color=>true), mime, x)
        end
        close(b64)
    end
